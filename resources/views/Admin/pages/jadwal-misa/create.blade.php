...
@section('page_title', 'Tambah Jadwal Misa')
@section('breadcrumb', 'Pengaturan / Jadwal Misa / Tambah')

@section('content')
    <form action="{{ route('admin.jadwal-misa.store') }}" method="POST">
        @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="hari_group" id="hari_group"
                            ...
                            @foreach (\App\Models\JadwalMisa::HARI_GROUPS as $h)
                                <option value="{{ $h }}" {{ old('hari_group') === $h ? 'selected' : '' }}>
                                    {{ $h }}</option>
                            @endforeach
                        </select>
                        @error('hari_group')
