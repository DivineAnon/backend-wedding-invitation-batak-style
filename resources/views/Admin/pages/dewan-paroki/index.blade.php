...
@section('title', 'Dewan Paroki')
@section('page_title', 'Dewan Paroki')
@section('breadcrumb', 'Konten / Dewan Paroki')

@section('topbar_actions')
    <a href="{{ route('admin.dewan-paroki.page.edit') }}" class="btn btn-secondary" title="Edit hero image halaman Dewan Paroki">
    ...
    <a href="{{ route('admin.dewan-paroki.create') }}" class="btn btn-primary">+ Tambah Anggota</a>
@endsection

@section('content')
    <div class="card">
        <div class="card__header">
            <div class="card__title">Semua Anggota Dewan Paroki</div>
        </div>
        <div class="card__body" style="padding:0;">
            <table class="table admin-table" id="tbl-dewan-paroki" data-title="DewanParoki">
                <thead>
                    <tr>
                        <th>Foto</th>
                        <th>Nama</th>
                    ...
                    @forelse ($anggota as $a)
                        <tr>
                            <td>
                                ...
                                @else
                                    <div
                                        style="width:44px;height:52px;border-radius:4px;background:var(--bg-alt,#f0f0f0);display:flex;align-items:center;justify-content:center;font-size:18px;color:var(--muted);">
                                        —
                                    </div>
                                @endif
                            </td>
                            ...
                            <td>{{ $a->jabatan }}</td>
                            <td style="font-size:13px;">{{ $a->urutan }}</td>
                            <td style="text-align:right;">
                                <div style="display:flex;gap:6px;justify-content:flex-end;">
                                    <a href="{{ route('admin.dewan-paroki.edit', $a) }}" class="btn btn-secondary"
                                        style="padding:4px 10px;font-size:12px;">Edit</a>
                                    <form id="del-form-{{ $a->id }}" action="{{ route('admin.dewan-paroki.destroy', $a) }}" method="POST"
                                        style="display:none">
                                        @csrf @method('DELETE')
                                    </form>
                                    <button type="button" class="btn btn-danger"
                                        style="padding:4px 10px;font-size:12px;"
                                        onclick="openDelModal({{ $a->id }}, '{{ addslashes($a->nama) }}')">Hapus</button>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" style="text-align:center;color:var(--muted);padding:32px;">
                                Belum ada data anggota dewan paroki.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div id="modal-del"
        ...
        <div
            ...
            <h3 style="margin:0 0 8px;font-size:18px;font-weight:600;color:#333">Hapus Anggota</h3>
